...
public class SimulatorPDUProcessor extends PDUProcessor {
	 ...
	 */
	public SimulatorPDUProcessor(SMSCSession session, ShortMessageStore messageStore, Table users, Properties msisdnErrors) {
				...
				if (request.canResponse()) {
					...
					switch (commandId) { // for selected PDUs do extra steps
						case Data.SUBMIT_SM :
							...
							if (!checkStubbedError((SubmitSM) request, submitResponse)) {
	...
	private boolean checkStubbedError(SubmitSM request, SubmitSMResp response) {
		boolean error = false;
